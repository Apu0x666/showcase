const app = Vue.createApp({
    data() {
        return {
            moduleName: 'LossLog_CoefficientsTechInfo',
    ...
    methods: {
        ...
        handlerActionClick(event) {
            if (event.name === 'Edit') {
                this.$refs.techItemEditModal.editItem(event.row.id.value);
            }
            if (event.name === 'Delete') {
                this.$refs.techItemEditModal.deleteItem(event.row.id.value);
            }
        },
...
initComponents(app, ['LossLog/CoefficientsTechInfo']).then(() => app.mount('#app'));
