                ...
                <slot name="body">
                    <div class="tm-grid">
                        <div class="tm-width-1-1 child-flex-bottom">
                            <tm-input-number
                                v-model="planDeviationLeftValue"
                                disabled
                                title="Требуется пояснить отклонение потенциала от плана: "
                            ></tm-input-number>
                        </div>
...
export default {
    ...
    data() {
        return {
            ...
            atrList: [],
            planDeviationLeft: null,
            planDeviationLeftValue: null,
            doctype: 'atr',
    ...
    computed: {
        atrOptions() {
            if (this.doctype === 'atr') {
                // Для 'atr' возвращаем элементы, где is_arp === 0 И isNz === 0
    ...
    methods: {
        ...
        async newItem() {
            ...
            try {
                ...
                this.setDefaults();
                this.title = 'Добавление декомпозиции плана';
        ...
        calculatePlanDeviationLeft(initialDeviation, eventRow) {
            const isFlaringAccounting = eventRow['decompositionPlanItems-isFlaringAccounting']?.title ?? 0;
            const isOwnNeedsAccounting = eventRow['decompositionPlanItems-isOwnNeedsAccounting']?.title ?? 0;
        ...
        async getRelationsAndDeviationLeft(eventRow) {
            ...
            try {
                ...
                let data = await response.json();
                if (response.ok) {
                    const planDeviationLeftValue = parseFloat(data.planDeviationLeft);

                    self.planDeviationLeftValue = self.calculatePlanDeviationLeft(
                        planDeviationLeftValue,
                        eventRow,
                    );
                    self.planDeviationLeft = planDeviationLeftValue;
