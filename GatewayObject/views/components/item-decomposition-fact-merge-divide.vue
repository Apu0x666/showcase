...
export default {
    ...
    methods: {
        ...
        async mergeForm(factId) {
            BaseTemplate.showProgress();
            try {
                // Проверяем коэффициенты
                await this.checkCoefficients();
                
                this.title = 'Объединить декомпозицию факта';
                this.action = this.actionEdit;
        ...
        async save() {
            ...
            try {
                ...
                if (!response.ok) {
                    Notify.showError(data.errorMessage || 'Произошла ошибка.');
                    return;
                }
                ...
                if (this.itemData.mergeOrDivide === 'divide') {
                    await this.divide();
                } else if (this.itemData.mergeOrDivide === 'merge') {
