...
class LossLog_CoefficientsGeoInfo_Model extends Model_Extended implements LossLog_Interface
{
    use LossLog_Coefficients_Model_Trait;
    use Operations_Trait;

    protected string $table = 'losslog_coefficients_geo_info';

    // строго совпадает со значением справочника типов коэффициента
    public const int COEFFICIENT_TYPE = 1;
    ...
    public const ERROR_EMPTY_RESPONSIBLE = 10;
    public const ERROR_EMPTY_DATE_START = 11;
    public const ERROR_EMPTY_DATE_END = 12;
    public const ERROR_INVALID_DATE_RANGE = 13;
    public const ERROR_DATE_OVERLAP = 14;
     ...
     */
    public function getGeoCoefficientsByObjectId(string $date, int $objectId): array
    {
        $filter = [
            'date_start' => ['<=', $date],
            'date_end'   => ['>=', $date],
        ...
        $coefficients = $this->getList(
            $filter,
            '',
            0,
            0,
            'object_id,sikeling_pt,sikeling_bt,sikeling_sgk,sikeling_polyethylene,gf,water,gas'
        );
